...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const particlesRef = useRef<Set<HTMLSpanElement>>(new Set());
  ...
  return (
    <div className="relative" ref={containerRef}>
      <nav className="relative z-20">
        <ul ref={navRef} className="flex gap-8 list-none p-0 px-4 m-0 relative">
          {items.map((item, index) => (
            <motion.li
              key={index}
              className={`py-2 px-4 rounded-full relative cursor-pointer transition-colors duration-300 text-white/80 hover:text-white ${
      ...
      <span
        ref={textRef}
        className="absolute z-30 flex items-center justify-center text-white transition-colors duration-300"
      />
    </div>
  );
};
